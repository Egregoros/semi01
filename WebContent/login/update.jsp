...
<script type="text/javascript">
   ...
   function callback(){
      if(xhr.readyState==4 && xhr.status==200){
         ...
         if(id.length<4 && id.length>0 || id.length>12) {
            span.innerHTML="아이디는 4글자 이상 또는 11글자 이하로 입력해주세요.";
            return;
           }
   ...
   function checknickname(){
	   ...
	   function callback2(){
	      if(xhr.readyState==4 && xhr.status==200){
	         ...
	         if(nickname.length<2 && nickname.length>0 || nickname.length>10) {
   ...
   function backcall(){      
      if(xhr.readyState==4 && xhr.status==200){
         ...
         if(phone.length!=11){
            span.innerHTML="번호를 11자리로 입력해주세요.";
            return;
         }
